...
func UnmarshalJSON(data []byte, i interface{}) error {
	...
	if len(data) == 0 {
		return NewError("Bad Request", readerError, "elemental", http.StatusBadRequest)
	}
	...
	for k, v := range d {
		...
		// Should not go there
		if err != nil {
			errors = append(errors, NewError("Validation Error", typeError, "elemental", http.StatusUnprocessableEntity))
			continue
		}

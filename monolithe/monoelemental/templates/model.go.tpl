...
func (*{{specification.entity_name}}) SpecificationForAttribute(name string) {{ glob.prefix }}AttributeSpecification {
  ...
  if v, ok := {{ specification.entity_name }}AttributesMap[name]; ok {
    return v
  }

  // We could not find it, so let's check on the lower case indexed spec map
  return {{ specification.entity_name }}LowerCaseAttributesMap[name]
}
...
// {{ specification.entity_name }}LowerCaseAttributesMap represents the map of attribute for {{ specification.entity_name }}.
var {{ specification.entity_name }}LowerCaseAttributesMap = map[string]{{ glob.prefix }}AttributeSpecification{
  {% for attribute in specification.attributes %}
